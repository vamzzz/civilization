...
public class GameController {
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        if (state == GameState.ATTACKING) {
            if (last.getTile().isEmpty() || last.getTile()
                ...
                alert.setTitle("You cannot attack this tile");
                alert.showAndWait();

            } else {
                AudioClip attack1 = new AudioClip(
                    "File:./src/main/java/view/music/Sword.mp3");
                attack1.play();
                attack1.setVolume(.1);
                attack(last);
            }
            ...
            if (move(last)) {
                AudioClip move = new AudioClip(
                    "File:./src/main/java/view/music/move.mp3");
                move.play();
                move.setVolume(.1);
                lastClicked = last;
            }
    ...
    public static void ai() {
        ...
        for (int r = 0; r < GridFX.getMapSize(); r++) {
            for (int c = 0; c < GridFX.getMapSize(); c++) {
        ...
        if (banditHideout != null) {
            if (rand.nextInt(100) < GridFX.getMapSize()) {
        ...
        for (TerrainTile tile : bandits) {
            ...
            if (newRow >= 0 && newRow < GridFX.getMapSize()
                && newCol >= 0 && newCol < GridFX.getMapSize()) {
    ...
    private static TerrainTile getRandomAdjacentTile(int row, int col) {
        ...
        while (!(r >= 0 && r < GridFX.getMapSize()
            && c > 0 && c < GridFX.getMapSize())) {
